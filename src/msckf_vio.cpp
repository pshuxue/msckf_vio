...
namespace msckf_vio
{
  ...
  MsckfVio::MsckfVio(ros::NodeHandle &pnh) : is_gravity_set(false),
                                             is_first_img(true),
                                             nh(pnh)
  {
    return;
  ...
  bool MsckfVio::createRosIO()
  {
    ...
    imu_sub = nh.subscribe("imu", 100, //imu 的原始数据
                           &MsckfVio::imuCallback, this);
    feature_sub = nh.subscribe("features", 40, //图像节点发出的特征点数据，即同一个点在cam1和cam0中的像素坐标
  ...
  //初始化状态参数，使用函数createRosIO创建ros节点
  ...
  {
    //imu数据并不是马上被处理，而是先放入缓冲区，等下一帧图像来的时候再进行处理
    imu_msg_buffer.push_back(*msg);

    if (!is_gravity_set) //如果重力没有被初始化
    {
      if (imu_msg_buffer.size() < 200)
        return;
      initializeGravityAndBias(); //初始化重力向量
      is_gravity_set = true;
    }
  ...
  // 初始化重力向量和陀螺仪的偏置（运行之前应该要求该场景的图像在最开始保持静止不动）
  //重力向量的求解就是在静止状态下加速度记的均值，再求模值，再乘以(0,0,1)
  void MsckfVio::initializeGravityAndBias()
  {
    ...
    //将IMU的orientation（相对世界坐标的旋转四元数保存）
      ...
      Matrix<double, 4, 6> &H_x, Matrix<double, 4, 3> &H_f, Vector4d &r)
  {
    ...
    u.block<3, 1>(0, 0) = quaternionToRotation(
                              cam_state.orientation_null) *
                          IMUState::gravity;
    u.block<3, 1>(3, 0) = skewSymmetric(
                              p_w - cam_state.position_null) *
                          IMUState::gravity;
