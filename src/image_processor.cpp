...
namespace msckf_vio
{
  ...
  //初始化订阅者和发布者，指定两个回调函数stereoCallback和imuCallback
  bool ImageProcessor::createRosIO()
  {
    ...
    stereo_sub.connectInput(cam0_img_sub, cam1_img_sub); //将两个消息打包一起处理
  ...
  //图像处理节点的入口函数，加载参数，最终执行createRosIO
      ...
      const sensor_msgs::ImageConstPtr &cam1_img)
  {
    ...
    // 对两帧图像构建金字塔，分别存入curr_cam0_pyramid_和curr_cam1_pyramid_
    createImagePyramids();
  ...
  //利用两个相机的内外参进行特征匹配，输入cam0的点，输出cam1的匹配点和内点，能够加快匹配时间
      ...
      vector<unsigned char> &inlier_markers)
  {
    ...
    {
      //对于cam1而言，现在是没有特征点的，
      // 于是undistortPoints函数将cam0的特征点先是解畸变，并通过两个相机之间的旋转将解畸变以后的点变到另一坐标下（cam1坐标，但是没有考虑平移）
      //distortPoints函数在将畸变加上，也就得到了cam1下的图像坐标(但是还是没有考虑外参中的平移量)，所以这个坐标并不准
      //虽然没考虑平移不准，但是后面的光流跟踪能够找的准一些，这里只是给一个初值方便后面计算。
    ...
    //使用LK光流找到cam1的特征点（与cam0对应的那些特征点）存储在cam1_points
    ...
    //对两帧图像cam0和cam1解畸变（解畸变之后的点才满足本质矩阵）
  ...
  //将图像解畸变，但是还附加了一个参数就是rectification_matrix，
  //即在解畸变的时候可以将解后的点进行一次旋转，前4个参数是输入，第五个参数是解畸变的输出，
  //第6个参数是输入的旋转矩阵，第7个参数全程没有用上
